...
OUTPUT = 'jobs_results_playwright_debug.csv'
...
def scrape_jobbank(page):
    ...
    page.screenshot(path='jobbank_search.png')
    anchors = page.query_selector_all('a[href*="/jobsearch/jobposting/"]')
    print('Found', len(anchors), 'posting anchors')
    seen = set()
    for i, a in enumerate(anchors[:20]):
        try:
            ...
            # save screenshot per item
            page.screenshot(path=f'jobbank_posting_{i}.png')
...
def scrape_indeed(page):
    ...
    page.screenshot(path='indeed_search.png')
    # try to click first few cards
    ...
    for i, c in enumerate(cards[:20]):
        try:
            ...
            page.goto(link)
            page.wait_for_timeout(1500)
            page.screenshot(path=f'indeed_posting_{i}.png')
            title = page.title()
...
def main():
    with sync_playwright() as p:
        ...
        if all_results:
            ...
            print('No results found by Playwright debug scraper')
        browser.close()
